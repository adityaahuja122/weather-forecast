...
def signup():
    if firstName_entry.get() == "" or lastName_entry.get() == "" or passwordName_entry.get() =="" or emailName_entry.get() == "" or confirm_passwordName_entry.get() == "":
        ...
        try :
            ...
            cur.execute("INSERT INTO AccountDB(FirstName, LastName, Email, Password) VALUES(?,?,?,?)",(firstName_entry.get(),lastName_entry.get(),emailName_entry.get(),passwordName_entry.get()))
            connection.commit()
            connection.close()
            clear()
...
def login():
    email_value = Login_emailName_entry.get()
    password_value = Login_passwordName_entry.get()
    print("Email:", email_value)
    print("Password:", password_value)
    def login_worker():
        conn = sqlite3.connect("./Database/AccountSystem.db")
        cursor = conn.cursor()
        # Fix the typo in the SQL query
        ...
        if result:
            ...
            subprocess.Popen(["python", "tryweatherapp.py"])
            # Close the entire application window upon successful login
            AccountSystem.quit()
        else:
    ...
    login_thread.start()
# Define the forgot_password function
def forgot_password():
    win = Toplevel()
    window_width = 350
    window_height = 350
